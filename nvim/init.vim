...
function! init#initGlobalVars()
    ...
    let g:plugin_load_hooks = [
                ...
                \ ]
    " 使用telescope (1=telescope用leader键, 0=fzf用leader键)
...
" ssh 远程剪贴板: Neovim 0.10+ 自动支持 OSC 52，无需额外配置
" WezTerm + tmux (set-clipboard on) 原生支持 OSC 52
if !empty($SSH_TTY) && !executable('clipboard-provider')
    " SSH 环境下 Neovim 0.10+ 自动使用 OSC 52，无需手动设置
elseif executable('clipboard-provider')
    let g:clipboard = {
          \ 'name': 'myClipboard',
          \     'copy': {
...
" 增加依赖项检测（仅 ArchLinux 自动安装，其他系统直接跳过）
