...
augroup autoRunGroup
    ...
    " fcitx-remote 在 SSH 远程环境下无效，仅本地环境执行
    if empty($SSH_TTY)
...
" 使用 $SHELL（zsh），仅 fish 回退到 bash（fish 语法不兼容 POSIX）
let s:user_shell = environ()['SHELL']
if s:user_shell =~ 'fish'
    set shell=bash
    set shellcmdflag=-lc
else
    let &shell = s:user_shell
    set shellcmdflag=-lc
endif
